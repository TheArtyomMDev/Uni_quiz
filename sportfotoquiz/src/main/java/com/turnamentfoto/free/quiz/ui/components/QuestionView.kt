...
import com.turnamentfoto.free.quiz.ui.theme.compactFontFamily
...
fun TopicHeader(topic: QuizTopic, onHomeClick: () -> Unit) {
    val picture = when(topic) {
        is QuizTopic.Photo -> painterResource(R.drawable.header_photo)
        is QuizTopic.Leaugue -> painterResource(R.drawable.header_leaugue)
    }
    Box {
        Image(
            painter = picture,
...
fun QuestionText(question: Question, questionIndex: Int) {
    Box() {
        Spacer(Modifier.height(300.dp).fillMaxWidth().align(Alignment.Center))
